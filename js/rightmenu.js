...
window.oncontextmenu = function(event){
    ...
    if(document.getSelection().toString()){   /* 选中文本后 */
      ...
      if(document.getElementById('post-comment')) {
        $('#rightMenu-reference').show();
      }else {
        $('#rightMenu-reference').hide();
      }
...
kk.copySelect = function(){
  ...
  //这里可以写点东西提示一下 已复制
  btf.snackbarShow('复制成功，复制和转载情标注本文地址');
}

//引用到评论
function reference(text) {
  if(text) {
    document.getElementsByClassName('el-textarea__inner')[0].value = '> ' + text.toString().trim() + '\n>\n';
    ...
    document.getElementsByClassName('el-textarea__inner')[0].value = '> ' + valueStr.toString().trim() + '\n>\n';
    document.getElementsByClassName('el-textarea__inner')[0].focus();
